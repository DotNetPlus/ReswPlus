...
namespace ReswPlusLibUnitTests
{
    ...
    public class HtmlParserUnitTest
    {
        ...
        public void TestHTMLParserBasicString()
        {
            ...
            Assert.IsTrue(CheckRun((Run)res[0], "Hello", FontWeights.Normal, FontStyle.Normal, TextDecorations.None));
            ...
            Assert.AreEqual(res.Count(), 1);
            Assert.IsTrue(CheckRun((Run)res[0], "  Hello World ", FontWeights.Normal, FontStyle.Normal, TextDecorations.None));

            res = HTMLParser.Parse(" ").ToList();
            Assert.AreEqual(res.Count(), 1);
            Assert.IsTrue(CheckRun((Run)res[0], " ", FontWeights.Normal, FontStyle.Normal, TextDecorations.None));
        ...
        public void TestHTMLParserSimpleEmphasis()
        {
            ...
            Assert.AreEqual(res.Count(), 1);
            Assert.IsTrue(CheckRun((Run)res[0], "  Hello", FontWeights.Normal, FontStyle.Italic, TextDecorations.None));

            res = HTMLParser.Parse("<u>Hello  </u>").ToList();
            Assert.AreEqual(res.Count(), 1);
            Assert.IsTrue(CheckRun((Run)res[0], "Hello  ", FontWeights.Normal, FontStyle.Normal, TextDecorations.Underline));

            res = HTMLParser.Parse("<s>  Hello World </s>").ToList();
            Assert.AreEqual(res.Count(), 1);
            Assert.IsTrue(CheckRun((Run)res[0], "  Hello World ", FontWeights.Normal, FontStyle.Normal, TextDecorations.Strikethrough));
        }
