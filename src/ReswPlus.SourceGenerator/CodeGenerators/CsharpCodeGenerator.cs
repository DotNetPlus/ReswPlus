...
internal sealed class CSharpCodeGenerator : ICodeGenerator
{
    ...
    /// <summary>
    /// Returns the token declaring a member or a parameter named after the content of a resource file.
    /// </summary>
    /// <param name="name">The name as it is written in the resource file.</param>
    /// <returns>The identifier to emit.</returns>
    private static SyntaxToken MemberIdentifier(string name)
    {
        return Identifier(GeneratedIdentifier.Escape(name));
    }

    /// <summary>
    /// Returns the reference to a member or a parameter named after the content of a resource file.
    /// </summary>
    /// <param name="name">The name as it is written in the resource file.</param>
    /// <returns>The name to emit.</returns>
    private static IdentifierNameSyntax MemberName(string name)
    {
        return IdentifierName(GeneratedIdentifier.Escape(name));
    }
    ...
    /// <summary>
    /// Returns the namespace of the XAML framework the generated code binds to.
    /// </summary>
    /// <param name="appType">The kind of app the code is generated for.</param>
    /// <returns>The namespace, qualified with <c>global::</c>.</returns>
    /// <remarks>
    /// The generated types are emitted inside the namespace of the consumer, and a type declared there takes
    /// precedence over one a using directive brings in. A project that happens to declare its own
    /// <c>MarkupExtension</c> or <c>IValueConverter</c> would otherwise silently rebind the generated markup
    /// extension to it, so the framework types are named in full.
    /// </remarks>
    private static string XamlNamespace(AppType appType)
    {
        return appType == AppType.WindowsAppSDK ? "global::Microsoft.UI.Xaml" : "global::Windows.UI.Xaml";
    }
    ...
    private ClassDeclarationSyntax CreateStronglyTypedClass(StronglyTypedClass info)
    {
        ...
        var resourceField = FieldDeclaration(
                VariableDeclaration(ParseTypeName("global::_ReswPlus_AutoGenerated.ResourceStringProvider"))
            ...
            .WithBody(Block(
                ExpressionStatement(
                    AssignmentExpression(
                        ...
                        ObjectCreationExpression(ParseTypeName("global::_ReswPlus_AutoGenerated.ResourceStringProvider"))
                            .WithArgumentList(
                                ArgumentList(
                                    SingletonSeparatedList(
                                        Argument(
    ...
    private IEnumerable<MemberDeclarationSyntax> CreateFormatMethodSyntax(Localization item)
    {
        ...
        if (item.IsProperty)
        {
            ...
            var propertyDecl = PropertyDeclaration(PredefinedType(Token(SyntaxKind.StringKeyword)), MemberIdentifier(item.Key))
                .WithModifiers(TokenList(
                    Token(SyntaxKind.PublicKeyword),
                    Token(SyntaxKind.StaticKeyword)
                ))
        ...
        else
        {
            ...
            if (functionParameters.Any(p => p.IsVariantId))
            {
                ...
                var overloadParameters = SeparatedList<ParameterSyntax>(
                    functionParameters.Select(p =>
                        Parameter(MemberIdentifier(p.Name))
                    ...
                    functionParameters.Select(p =>
                        Argument(
                            ...
                            ? InvocationExpression(
                                MemberAccessExpression(
                                    SyntaxKind.SimpleMemberAccessExpression,
                                    ParseExpression("global::System.Convert"),
                                    IdentifierName("ToInt64")
                                ),
                                ArgumentList(SingletonSeparatedList(Argument(MemberName(p.Name))))
                              )
                            : MemberName(p.Name)
                        )
                    )
                ...
                var overloadMethod = MethodDeclaration(PredefinedType(Token(SyntaxKind.StringKeyword)), MemberIdentifier(item.Key))
                    .WithModifiers(TokenList(
                        Token(SyntaxKind.PublicKeyword),
                        Token(SyntaxKind.StaticKeyword)
                    ))
                            ...
                            TryStatement(
                                Block(
                                    SingletonList<StatementSyntax>(
                                        ReturnStatement(
                                            InvocationExpression(MemberName(item.Key))
                ...
                var stdMethod = MethodDeclaration(PredefinedType(Token(SyntaxKind.StringKeyword)), MemberIdentifier(item.Key))
                    .WithModifiers(TokenList(
                        Token(SyntaxKind.PublicKeyword),
                        Token(SyntaxKind.StaticKeyword)
                    ))
            ...
            else
            {
                ...
                var methodDecl = MethodDeclaration(PredefinedType(Token(SyntaxKind.StringKeyword)), MemberIdentifier(item.Key))
                    .WithModifiers(TokenList(
                        Token(SyntaxKind.PublicKeyword),
                        Token(SyntaxKind.StaticKeyword)
                    ))
    ...
    private IEnumerable<StatementSyntax> GenerateFormatMethodBody(Localization item)
    {
        ...
        if (item is IVariantLocalization variantLoc && variantLoc.ParameterToUseForVariant != null)
        {
            ...
            keyExpr = BinaryExpression(
                ...
                MemberName(variantLoc.ParameterToUseForVariant.Name)
            );
        }
        else
        {
        ...
        if (item is PluralLocalization pluralLoc && pluralLoc.ParameterToUseForPluralization != null)
        {
            ...
            ExpressionSyntax pluralParamExpr = MemberName(pluralLoc.ParameterToUseForPluralization.Name);
            if (pluralLoc.ParameterToUseForPluralization.TypeToCast.HasValue)
            {
                ...
                pluralParamExpr = ParenthesizedExpression(
                    CastExpression(
                        ...
                        MemberName(pluralLoc.ParameterToUseForPluralization.Name)
                    )
                );
            }
            localizationExpr = InvocationExpression(
                MemberAccessExpression(
                    SyntaxKind.SimpleMemberAccessExpression,
                    ParseExpression("global::_ReswPlus_AutoGenerated.Plurals.ResourceLoaderExtension"),
                ...
                item.Parameters.Select<IFormatTagParameter, ExpressionSyntax>(p =>
                {
                    switch (p)
                    {
                        case FunctionFormatTagParameter funcParam:
                            return MemberName(funcParam.Name);
                        case MacroFormatTagParameter macroParam:
                            return MemberAccessExpression(
                                SyntaxKind.SimpleMemberAccessExpression,
                                ParseExpression("global::_ReswPlus_AutoGenerated.Macros"),
                        ...
                        case StringRefFormatTagParameter stringRefParam:
                            return MemberName(stringRefParam.Id);
                        default:
    ...
    private ClassDeclarationSyntax CreateMarkupExtensionSyntax(string resourceFileName, string className, IEnumerable<string> keys, AppType appType)
    {
        ...
        [
            ...
            AttributeList(
                SingletonSeparatedList(
                    Attribute(ParseName($"{XamlNamespace(appType)}.Markup.MarkupExtensionReturnType"))
                    .WithArgumentList(
                        AttributeArgumentList(
                            SingletonSeparatedList(
                                AttributeArgument(
        ...
        {
            enumMembers.Add(
                EnumMemberDeclaration(MemberIdentifier(key))
        ...
        var markupDecl = ClassDeclaration(className)
            ...
            .WithBaseList(
                BaseList(
                    SingletonSeparatedList<BaseTypeSyntax>(
                        SimpleBaseType(ParseTypeName($"{XamlNamespace(appType)}.Markup.MarkupExtension"))
                    )
                )
            )
            .WithAttributeLists(attributes)
            ...
            .AddMembers(
                ...
                PropertyDeclaration(ParseTypeName($"{XamlNamespace(appType)}.Data.IValueConverter"), "Converter")
                    .WithModifiers(TokenList(Token(SyntaxKind.PublicKeyword)))
                    .WithAccessorList(
                        AccessorList(
                            List(
